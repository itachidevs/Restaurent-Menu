...
const foodMenu = menu.map(item => {
  let listItem =
    `<div class="menu-item ${item.category} ${item.title} col-12 col-md-4 col-lg-3 m-3">
  <div class="card">
...
searchInput.addEventListener('keyup', (event) => {
  ...
  let stored = document.getElementById('cardsContainer')
  let cards = document.querySelectorAll('.menu-item');
  let title = document.getElementsByTagName('h5');
  let category = document.getElementsByTagName('small');
  for (var i = 0; i < title.length || i < category.length; i++) {
    let match1 = cards[i].getElementsByTagName('h5')[0];
    let match2 = cards[i].getElementsByTagName('small')[0];
    let matching1=match1.innerHTML||match1.textContent
    let matching2=match2.innerHTML||match2.textContent
    if(matching1.toLowerCase().startsWith(searchValue ) || matching2.toLowerCase().startsWith(searchValue )) {
